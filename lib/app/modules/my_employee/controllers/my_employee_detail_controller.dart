...
class MyEmployeeDetailController extends GetxController {
  ...
  final eEService = <EService>[].obs;
  final eEServiceA = <EService>[].obs;
  var eSelectService = <String>[].obs;
  var userId = "".obs;
  ...
  Future getAllServices() async {
    try {
      eEService.assignAll(await _eProviderRepository.getServiceList());
    } catch (e) {
      Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
    }
  }
  ...
  List<MultiSelectDialogItem<EService>> getMultiSelectServicesItems() {
    return eEService.map((element) {
      return MultiSelectDialogItem(element, element.name);
    }).toList();
  }

  Future getSubCategories(params) async {
    try {
      subCategories
          .assignAll(await _categoryRepository.getAllSubCategories(params));
    } catch (e) {
      Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
    }
  }
  ...
  List<SelectDialogItem<User>> getMultiSelectEmployeesItemsA() {
    return employees.map((element) {
  ...
  * */
  ...
  void createEProviderForm() async {
    ...
    try {
      ...
      //eProvider.value.services = eSelectService;
      eProvider.value.type = EProviderType(id: "3", name: "Freelancer");
      eProvider.value.description = "Employee";
      eProvider.value.name = employees[0].name;
      eProvider.value.services = eSelectService;
      eProvider.value.services = eEServiceA?.map((((e) => e.id)))?.toList();
